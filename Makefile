...
jar.name=triangle-repartitionjoin-1.0.jar
jar.path=target/${jar.name}
job.name=twitter.triangle
local.input=input
local.output=output
# Pseudo-Cluster Execution
hdfs.user.name=xuel12
hdfs.input=input
hdfs.output=output
# AWS EMR Execution
aws.emr.release=emr-5.26.0
aws.region=us-east-1
aws.bucket.name=aws-logs-354124988676-us-east-1-${job.name}
...
aws: jar upload-app-aws delete-output-aws
	aws emr create-cluster \
		--name "${job.name} RepartitionJoin Cluster" \
